...
Sim3Solver::Sim3Solver(std::shared_ptr<KeyFrame> pKF1, std::shared_ptr<KeyFrame> pKF2, const vector<std::weak_ptr<MapPoint> > &vpMatched12, const bool bFixScale)
    :
    mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale)
{
    ...
    vector<std::weak_ptr<MapPoint> > vpKeyFrameMP1 = pKF1->GetMapPointMatches();
    ...
    for (int i1 = 0; i1 < mN1; i1++) {
        if (!vpMatched12[i1].expired()) {
            std::weak_ptr<MapPoint> pMP1 = vpKeyFrameMP1[i1];
            std::weak_ptr<MapPoint> pMP2 = vpMatched12[i1];

            if (pMP1.expired() || pMP2.expired())
                continue;
            std::shared_ptr<MapPoint> spMP1 = pMP1.lock();
            std::shared_ptr<MapPoint> spMP2 = pMP2.lock();
            if (spMP1->isBad() || spMP2->isBad())
                continue;

            int indexKF1 = spMP1->GetIndexInKeyFrame(pKF1);
            int indexKF2 = spMP2->GetIndexInKeyFrame(pKF2);
            ...
            cv::Mat X3D1w = spMP1->GetWorldPos();
            mvX3Dc1.push_back(Rcw1 * X3D1w + tcw1);

            cv::Mat X3D2w = spMP2->GetWorldPos();
