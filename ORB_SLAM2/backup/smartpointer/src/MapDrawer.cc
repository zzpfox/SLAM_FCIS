...
void MapDrawer::GeneratePointCloud(const vector<std::shared_ptr<KeyFrame> > &vpKFs, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
                                   int begin, int step)
{
    pcl::PointXYZ point;
    ...
    for (int i = begin; i < keyFrameNum; i += step) {
        ...
        std::stringstream ss;
        ss << "./DepthImages/KeyFrame-" << std::setw(8) << std::setfill('0') << pKF->mnId << ".png";
...
void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph)
{
    ...
    if (bDrawGraph) {
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            // Loops
            std::set<std::weak_ptr<KeyFrame>, std::owner_less<std::weak_ptr<KeyFrame> > > sLoopKFs = spKF->GetLoopEdges();
            for (set<std::weak_ptr<KeyFrame>, std::owner_less<std::weak_ptr<KeyFrame> > >::iterator sit = sLoopKFs.begin(), send = sLoopKFs.end(); sit != send; sit++) {
