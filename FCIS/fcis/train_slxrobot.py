...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    sdsdbs = []
    slx_sdsdb = []
    for slxrobot in ['slxrobot']:
        slx_mask = os.path.join(config.dataset.root_path, 'cache', slxrobot, 'Masks')
        gt_sdsdb_file = os.path.join(slx_mask, 'gt_sdsdb.pkl')
        with open(gt_sdsdb_file, 'rb') as f:
            slx_sdsdb.extend(pkl.load(f))
    approx_samples_per_class = len(slx_sdsdb)


    coco_sd = []
    for image_set in image_sets:
        if image_set == 'train2014':
            gt_sdsdb_file = os.path.join(config.dataset.root_path, 'cache', 'COCOMask', 'train2014', 'gt_sdsdb.pkl')
            if os.path.exists(gt_sdsdb_file):
                with open(gt_sdsdb_file, 'rb') as f:
                    coco_sd.append(pkl.load(f))
            else:
                train2014_sdsdb = load_gt_sdsdb(config.dataset.dataset, image_set, config.dataset.root_path,
                ...
                coco_sd.append(train2014_sdsdb)
        elif image_set == 'valminusminival2014':
            gt_sdsdb_file = os.path.join(config.dataset.root_path, 'cache', 'COCOMask', 'val2014', 'gt_sdsdb.pkl')
            if os.path.exists(gt_sdsdb_file):
                with open(gt_sdsdb_file, 'rb') as f:
                    coco_sd.append(pkl.load(f))
            else:
                val2014_sdsdb = load_gt_sdsdb(config.dataset.dataset, image_set, config.dataset.root_path,
                                              config.dataset.dataset_path,
                                              mask_size=config.MASK_SIZE, binary_thresh=config.BINARY_THRESH,
                                              result_path=final_output_path, flip=config.TRAIN.FLIP)
                with open(gt_sdsdb_file, 'wb') as f:
                    pkl.dump(val2014_sdsdb, f, protocol=pkl.HIGHEST_PROTOCOL)
                coco_sd.append(val2014_sdsdb)
    coco_sd = merge_roidb(coco_sd)
    # coco_sd_subset = [coco_sd[i] for i in random.sample(xrange(len(coco_sd)), min(2 * approx_samples_per_class, len(coco_sd)))]
    classes_num = dict(zip(range(len(classes)), [0] * len(classes)))
    classes_num[cls_to_id['floor']] = approx_samples_per_class
    classes_full = [False] * len(classes)
    classes_full[0] = True
    coco_sd_sampled = []
    for item in coco_sd:
        clses = item['gt_classes']
        continue_flag = False
        for idx in xrange(clses.size):
            if classes_full[clses[idx]]:
                continue_flag = True
                break
        if not continue_flag:
            for idx in xrange(clses.size):
                classes_num[clses[idx]] += 1
                if classes_num[clses[idx]] >= approx_samples_per_class:
                    classes_full[clses[idx]] = True
            coco_sd_sampled.append(item)

            if all(classes_full):
                break
    logger.info('Classes Training Instance Number Statistics: \n')
    for key, value in classes_num.iteritems():
        logger.info('%s: %d\n' % (id_to_cls[key], value))
        print('%s: %d' % (id_to_cls[key], value))
    sdsdbs.append(coco_sd_sampled)
    sdsdbs.append(slx_sdsdb)
    ...
    mod.fit(train_data, eval_metric=eval_metrics, epoch_end_callback=epoch_end_callback,
            ...
            batches_checkpoint=epoch_end_callback, num_batches_save_ckpt=2000)
